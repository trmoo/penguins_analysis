...
- **셀 종류** (`src/lessons/data.js`)
  - `code` 노트북의 빈칸 셀(채점) · `review` 앞 단계 코드(실행만) · `extra` 더 해 보기(채점, 진행률 따로)
    ⚠ 「더 해 보기」는 **4단계에만** 있다(`describe` 확인·공식으로 직접 정규화). 1~3단계 것은 수업에서 다루지 않아 지웠다(사용자 지시 2026-09-21).
    그래서 안내 탭 명령어 사전에서도 `tail`·`value_counts`·`loc` 줄을 뺐고 `shape` 는 3단계로 옮겼다.
...
| `npm test` | **707가지** — 진짜 pandas 대조(golden) · 차시 정답/다른 정답/틀린 답/빈칸 · 화면에 적은 숫자 · 오류 도움말 |
